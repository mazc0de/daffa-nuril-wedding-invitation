...
const Button: React.FC<ButtonProps> = ({ children, variant = 'primary', size = 'md', color, borderRadius = 'rounded-md', padding, className = '', style, ...props }) => {
    ...
    const customStyle: React.CSSProperties = {
        ...style,
        backgroundColor: variant !== 'outline' && variant !== 'ghost' ? color : undefined,
    ...
    return (
        ...
        ${sizeClasses[size]} 
        ${variantClasses[color ? 'custom' : variant]} 
        ${borderRadius.includes('rounded') ? borderRadius : ''} 
        ${className}
      `}
            style={customStyle}
            {...props}
