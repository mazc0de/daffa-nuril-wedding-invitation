...
const Typography = <T extends ElementType = 'p'>({ as, variant = 'p', fontFamily = 'brigesta', weight = 'normal', color = 'primary', size, className, style, children, ...props }: TypographyProps<T>) => {
    const Component = as || (variant === 'caption' ? 'span' : variant);
    const isThemeColor = ['primary', 'secondary', 'muted', 'accent'].includes(color as string);
    ...
    return (
        <Component
            className={cn(!size && variantStyles[variant], fontStyles[fontFamily], weightStyles[weight], isThemeColor && colorStyles[color as ThemeColor], className)}
            style={{
